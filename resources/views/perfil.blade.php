    ...
    <body>
        <div class="container my-5">
            ...
            <header class="Perfil text-center">
                ...
                <p class="bio col-8 mx-auto">Uma breve descrição sobre o autor como seus gostos, ocupação e visão para
                    futuro etc.
                </p>
            </header>
                    ...
                    <div class="row">
                        <div class="box mx-auto col-10 my-3">
                            ...
                            <div class="perfilUser d-flex align-items-center">
                                ...
                                <i class="bi bi-exclamation-triangle text-danger ms-auto dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <h2>{{ $value->titulo }}</h2>
                            <p class="post">{!! $postagem->conteudo !!}</p>

                            <div class="footerBox d-flex justify-content-between">
                                <div>
                                    <time
                                        class="published">{{ \Carbon\Carbon::parse($postagem->created_at)->diffForHumans() }}</time>
                                    ·
                                    <time
                                        class="published">{{ \Carbon\Carbon::parse($postagem->created_at)->format('d/m/Y') }}</time>
                                </div>
                                <div>
                                    <a href="{{ url('/blog/curtida/' . $postagem->id) }}"><i class="bi bi-heart"></i></a>
                                    {{ $value->curtidas->count() }}
                                    <a href="{{ url('/blog/postagem/' . $postagem->id) }}"><i class="bi bi-chat"></i></a>
